...
class YOLOEDetect(Detect):
    ...
    def forward(self, x: list[torch.Tensor], cls_pe: torch.Tensor, return_mask: bool = False) -> torch.Tensor | tuple:
        ...
        for i in range(self.nl):
            print(f"x[{i}].shape: {x[i].shape}") # 打印输入到 cv2[i] 和 cv3[i] 的特征图形状
            branch1 = self.cv2[i](x[i])
            branch2 = self.cv3[i](x[i])
            print(f"self.cv2[{i}].weight.shape: {self.cv2[i].conv.weight.shape}") # 打印 cv2[i] 卷积层的权重形状
            print(f"self.cv3[{i}].weight.shape: {self.cv3[i].conv.weight.shape}") # 打印 cv3[i] 卷积层的权重形状
            print(f"branch1.shape (after cv2[{i}]): {branch1.shape}")
            print(f"branch2.shape (after cv3[{i}]): {branch2.shape}")
