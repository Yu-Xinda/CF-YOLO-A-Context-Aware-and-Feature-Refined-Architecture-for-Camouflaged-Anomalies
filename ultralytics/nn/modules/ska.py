...
# 使用新的 AMP API
from torch.amp import custom_fwd, custom_bwd
# from torch.cuda.amp import custom_fwd, custom_bwd  # 旧版API，已弃用
...
class SkaFn(Function):
    @staticmethod
    @custom_fwd(device_type='cuda')  # 使用新的 API
    def forward(ctx, x: torch.Tensor, w: torch.Tensor) -> torch.Tensor:
        # 添加输入验证
        ...
        if x.device != w.device:
            raise ValueError(f"Input tensors must be on the same device. x: {x.device}, w: {w.device}")
        ...
        # 验证权重张量形状
        expected_w_shape = (n, wc, ks**2, h, width)
        if w.shape != expected_w_shape:
            raise ValueError(f"Weight tensor shape mismatch. Expected: {expected_w_shape}, Got: {w.shape}")
        ...
        try:
            ...
            raise RuntimeError(f"SKA forward kernel failed: {e}")
    ...
    @staticmethod
    @custom_bwd(device_type='cuda')  # 使用新的 API
    def backward(ctx, go: torch.Tensor) -> tuple:
        ks, pad = ctx.ks, ctx.pad
        x, w = ctx.saved_tensors
        n, ic, h, width = x.shape
        ...
        if ctx.needs_input_grad[0]:
            ...
            try:
                ...
                raise RuntimeError(f"SKA backward_x kernel failed: {e}")
        ...
        if ctx.needs_input_grad[1]:
            ...
            try:
                ...
                raise RuntimeError(f"SKA backward_w kernel failed: {e}")
