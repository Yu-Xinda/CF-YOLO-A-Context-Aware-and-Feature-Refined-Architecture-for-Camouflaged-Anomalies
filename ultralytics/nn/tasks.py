...
class BaseModel(torch.nn.Module):
    ...
    def _predict_once(self, x, profile=False, visualize=False, embed=None):
        ...
        for m in self.model:
            ...
            if isinstance(m, MSCBlock):
                if isinstance(x, list) and len(x) == 2:
                    x, y_feature = x  # 直接拆出来
                else:
                    y_feature = y[m.idx_y] if m.idx_y < len(y) else None
...
class DetectionModel(BaseModel):
    ...
    def __init__(self, cfg="yolo11n.yaml", ch=3, nc=None, verbose=True):
        ...
        #         [s / x.shape[-2] for x in _forward(torch.zeros(1, ch, s, s, device=next(self.parameters()).device))],
        ...
        if isinstance(m, Detect):  # includes all Detect subclasses like Segment, Pose, OBB, YOLOEDetect, YOLOESegment
            ...
            # Ensure model is in eval/train states for stride calc
            self.model.eval()
            m.training = True

            # --- Robust device handling & debug prints ---
            # prefer device of last module parameters; fallback to first param of self; fallback to cuda if available
            try:
            ...
            # If CUDA is available but model is on CPU, try to move model to cuda explicitly
            if torch.cuda.is_available() and device.type == "cpu":
                try:
                    self.to(torch.device("cuda"))
                    device = next(m.parameters()).device  # refresh device
                    print(f"[stride setup] moved model to {device}")
                except Exception as e:
                    print(f"[stride setup] failed to move model to CUDA: {e}. Continuing with device={device}")

            # create test input on the chosen device
            x_tmp = torch.zeros(1, ch, s, s, device=device)

            # debug: show devices
            print(f"[stride setup] model device: {device}, input device: {x_tmp.device}")

            try:
                # compute stride using tensors on same device; ensure result on CPU (torch.tensor default CPU) if needed
                strides_list = [s / x.shape[-2] for x in _forward(x_tmp)]
                m.stride = torch.tensor(strides_list, device=device)
                # move stride to cpu tensor if rest of code expects cpu
                try:
                    m.stride = m.stride.cpu()
                except Exception:
                    pass
                self.stride = m.stride
            except Exception as e:
                # helpful debug output to locate offending tensor
                print("[stride setup] ERROR during forward used to compute strides:", e)
                print("-> torch.cuda.is_available():", torch.cuda.is_available())
                # Inspect some important tensors/devices in DSConv / SKA path if possible
                # (we attempt a forward and inspect devices of top-level tensors)
                try:
                    out = _forward(x_tmp)
                    if isinstance(out, (list, tuple)):
                        for i, xi in enumerate(out):
                            print(f"out[{i}] device: {getattr(xi, 'device', 'no device')}, shape: {getattr(xi, 'shape', None)}")
                    else:
                        print(f"out device: {getattr(out, 'device', 'no device')}, shape: {getattr(out, 'shape', None)}")
                except Exception as e2:
                    print("[stride setup] forward failed when attempting diagnostics:", e2)
                # Re-raise so upstream caller still fails visibly (you can remove to continue)
                raise
...
def parse_model(d, ch, verbose=True):
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in base_modules:
            ...
            # 确保通道数满足DSBlock的要求（能被8整除，最小为8）
            c1 = ch[f]
            if c1 < 8:
                c1 = 8  # 最小通道数
            elif c1 % 8 != 0:
                c1 = ((c1 + 7) // 8) * 8  # 对齐到8的倍数
            args = [c1]  # DSConv 只需要输入通道数 (dim=c1)，其他参数在类内部固定
        elif m is AIFI:
            args = [ch[f], *args]
            ...
            if isinstance(f, list) and len(f) == 2:
                c1, c2 = ch[f[0]], ch[f[1]]  # 分别取 x, y 的通道
                args = [c1, c2] + args        # 构造 MSCBlock 参数列表
                ch_out = c1                   # MSCBlock 输出通道对齐 x
            else:
                raise ValueError("MSCBlock needs exactly 2 inputs")
        else:
            c2 = ch[f]
